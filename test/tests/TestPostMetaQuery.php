...
class TestPostMetaQuery extends  Artovenry\CustomPostType\TestCase{
  function setup(){
    parent::setup();
    //{title: Title:1 show_at_home: 5 scheduled_on: 2015-12-31}
    //{title: Title:2 show_at_home: 4}
    //{title: Title:3 show_at_home: 3 scheduled_on: 2015-12-31}
    //{title: Title:4 show_at_home: 2}
    //{title: Title:5 show_at_home: 1 scheduled_on: 2015-12-31}
    foreach([1,2,3,4,5] as $i){
      $id= $this->insert("type_one", "Title:$i");
      TypeOne::build($id)->set("show_at_home", 6 - $i);
      if($i % 2 === 1)TypeOne::build($id)->set("scheduled_on", "2015-12-31");
    }
  }
